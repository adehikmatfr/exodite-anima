...
String hexKey(int seed) =>
    List.generate(32, (i) => ((i * 7 + seed) & 0xff).toRadixString(16).padLeft(2, '0')).join();
...
void main() {
  ...
  test('S1: cipher build is the one loaded', () {
    ...
    print('sqlite_version=$v cipher_rows=${ciphers.map((r) => r.values).toList()}');
  ...
  test('S1: create, write, close, reopen with right key, refuse wrong key', () {
    ...
    unlock(db, good);
    db.execute('CREATE TABLE entry(id INTEGER PRIMARY KEY, day TEXT, body TEXT)');
    db.execute('INSERT INTO entry(day, body) VALUES (?, ?)', ['2026-09-20', 'hello $marker']);
    db.close();
    ...
    db = sqlite3.open(path);
    unlock(db, bad);
    expect(() => db.select('SELECT body FROM entry'), throwsA(isA<SqliteException>()));
    db.close();
  ...
  test('S2: full-text search works inside the encrypted file, and no plaintext hits disk', () {
    final key = hexKey(3);
    final db = sqlite3.open(path);
    unlock(db, key);
    db.execute('PRAGMA journal_mode = WAL;');
    db.execute('CREATE VIRTUAL TABLE entry_fts USING fts5(body)');
    db.execute('INSERT INTO entry_fts(body) VALUES (?), (?)', ['walk in the park $marker', 'quiet evening']);
    final hits = db.select("SELECT body FROM entry_fts WHERE entry_fts MATCH 'park'");
    expect(hits.length, 1);

    // Inspect every file the database owns while it is still open (WAL not yet checkpointed).
    final files = dir.listSync().whereType<File>().toList();
    print('files while open: ${files.map((f) => f.uri.pathSegments.last).toList()}');
    for (final f in files) {
      expect(containsBytes(f.readAsBytesSync(), marker), isFalse, reason: f.path);
      expect(containsBytes(f.readAsBytesSync(), 'quiet evening'), isFalse, reason: f.path);
    }
    db.close();
  ...
  test('S2: plain SQLite control shows the inspection method can find plaintext', () {
    final db = sqlite3.open(path);

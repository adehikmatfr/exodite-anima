...
## Update 2026-09-23 (FEAT-011: real-device photo bugs - permission and the app lock)
The owner's first real-device test of FEAT-011 found two bugs neither the repository/unit/widget suite nor this ADR's earlier "no extra permission beyond a `FileProvider`" reading caught:

1. **Creating a new entry could not reach the camera or the library at all.** Cause: `EditorPage._addPhoto` called `_ensureEntryId()` (which needs saved, non-blank text) *before* ever calling the picker, so on a still-blank new entry the picker never opened - not a permission problem, a silent no-op. Fixed in `app/lib/journal/editor_page.dart`: a blank entry now shows `S.photoNeedsTextFirst` (a new banner, reusing `_PhotoAddError`'s inline pattern) instead of doing nothing. This ADR's schema and file-layer decisions are unaffected: an entry still needs saved text before a photo can attach to it (FEAT-001 AC-8's "no empty entry is saved" rule stands), only the feedback changed.
2. **Editing an existing entry could reach the camera and library, but the added photo then vanished**, thrown back to the timeline list. Cause: opening the camera or the library backgrounds the app exactly the way switching to any other app does, and FEAT-003's shell (`_ShellState` in `app/lib/main.dart`) locks *immediately* on backgrounding by default (`lockTimeout: Duration.zero`) - closing the database, wiping the data key, and popping every screen back to the timeline before `image_picker`'s result (delivered only after the app resumes) could ever reach `EntryRepository.addPhoto`. This is the same interaction the workflow doc's export/share gotcha already named ("anything a screen does after coming back from another screen can run after the app locked") - photos are different only in that there is nowhere left to write the result once locked, unlike a fire-and-forget refresh.

   Fixed with a small, explicit mechanism rather than by loosening the lock rule itself: `AutoLockGuard` (`app/lib/lock/auto_lock_guard.dart`), a hold count `EditorPage._addPhoto` wraps its whole picker-and-save operation in. `_ShellState._lock()` still fires on the same triggers as before (backgrounding, or the configured timeout elapsed) but, while a hold is active, defers instead of running, and applies the deferred lock the moment the last hold releases (`_onAutoLockGuardChanged`). The passcode is still asked for again after using the camera or the library, unchanged; the difference is only that the photo is written to the still-open, still-keyed database *before* that lock, instead of being lost when the lock ran too early. Proven in `app/test/auto_lock_guard_test.dart`: the guard's hold/release/exception behaviour in isolation, and an integration case pumping the real `JournalApp` shell that shows a held photo pick keeps `TimelinePage` on screen through a full background/resume cycle and still locks (`LockPage`) the instant the hold ends.

   **Residual risk, recorded rather than silently accepted:** while a hold is active, someone who gets hold of the unlocked phone during the brief window the user is actually inside the camera or library app (not our own app) could back out into a still-unlocked `EditorPage` without a passcode prompt. The screen-privacy cover and `FLAG_SECURE` still apply throughout (they are not part of what is held off), so nothing is exposed in the recents/app-switcher snapshot; the exposure is bounded to that one screen, for at most the time the picker takes to return. This trade-off was made to fix a data-loss bug the owner reported directly, not independently reviewed by cyber-security yet - flagged in `cyber-security/report/threat-model-v1.md` and the permission register below for that review before the next release sign-off.

**Permissions (FEAT-011, real-device finding):** `AndroidManifest.xml` had no `CAMERA` permission; on the device tested this is exactly why the camera path (separately from bug 1 above) could not be reached reliably. Added `<uses-permission android:name="android.permission.CAMERA"/>` and `<uses-feature android:name="android.hardware.camera" android:required="false"/>`; `image_picker`'s own Android code (`ImagePickerUtils.isPermissionPresentInManifest`, `ImagePickerDelegate`) checks the manifest for exactly this permission and, only when present, requests it at runtime just before opening the camera - confirmed by reading the plugin's source, not assumed. The photo library needed no equivalent addition: nothing in `image_picker`'s Android code requests a storage permission for it (Android 13+ uses the system Photo Picker, which needs none at all; older versions read through `ACTION_GET_CONTENT`, serviced by the gallery app itself), so declaring `READ_MEDIA_IMAGES`/`READ_EXTERNAL_STORAGE` would sit unused and unjustified (`frontend-mobile/report/permission-and-privacy-register.md`'s own "no justification, no permission" rule). `tool/check_release_manifest.sh`'s allow-list updated to include `CAMERA`, per the project rule that a new permission needs an ADR (this update) before a line in that allow-list.

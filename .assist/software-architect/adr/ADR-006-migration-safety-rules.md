...
## Update 2026-09-23 (rules 1 and 2 implemented)
Rule 1 (copy before a migration, delete only after the next successful launch) and rule 2 (restore and refuse to leave a half-migrated file on failure) are now built in `app/lib/data/journal_database.dart` (`backupBeforeMigration`, `restoreFromBackup`, `cleanupOldMigrationBackup`, used by `openEncrypted`). Rule 3 (never write a newer schema) was already built (`JournalTooNewException`). Tested in `app/test/migration_safety_test.dart` (TC-100, Partial): with the schema still at version 1, the failure path is proven by making a file claim an older version with no `onUpgrade` step defined for it, which makes drift's own migration fail for a real reason, not a simulated one; the entry data is confirmed intact afterward by a raw read. This is not yet tested against a real second schema, and rule 4 (transactional or resumable large migrations) has nothing to apply to yet, since no `onUpgrade` step exists. Both remain the responsibility of whichever `FEAT-` first bumps `journalSchemaVersion` (FEAT-010 is next in line).

...
| 14 | Dependencies | Licences, failure modes understood | R | Partial | docs | All reviewed packages have permissive licences (`dependency-review.md`); versions pinned except two; automated vulnerability scan added 2026-09-23 (`.github/workflows/osv-scanner.yml`, OSV-Scanner) but not yet run (needs the first push); the SQLite build hook downloads a prebuilt binary at build time and checks its hash | frontend-mobile: push and confirm the scan runs clean |

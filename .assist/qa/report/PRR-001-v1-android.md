...
| 6 | Security | Threat model reviewed; scans clean; authn tested | B | **Partial** | docs, local | THR-001..015 (self-reviewed); wrong-passcode wait, passcode change, wrapped key, encrypted export, hostile-file tests pass; open blockers in `security-requirements-review-v1.md`: G1 (key-derivation settings need a phone), G10 (CI manifest check written, not yet run), G11 (iOS backup, out of scope); no dependency vulnerability scan; device-bound key wrapping not evaluated (spike S5); no independent review | first CI run; a physical phone; an independent reviewer (RISK-005) |
...
| 10 | Release | Repeatable release; rollback written; migration reversible | R | Partial | docs, local | Staged rollout with a halt rule decided; CI workflow written, never run; **no release signing key** (TC-091); the first data migration (ADR-006) has no code or test yet (schema is still version 1, nothing released) | project owner: release key and Play account; first CI run |
| 11 | Capacity | | - | N/A | | no server | |
| 12 | Compliance | Applicable rows of `compliance-matrix.md` satisfied | B** | **Partial** | docs, local | WCAG 2.2 AA (adopted): contrast computed for 17 pairs, sizes by construction, 200% text tested on some screens; screen-reader, contrast and all-screen text-size runs not done (TC-087 to TC-089). Store rules: honest answer is "no data collected"; the forms and the privacy statement page are not written. Legal: self-assessment written, owner sign-off pending (`legal-self-assessment-v1.md`). Indonesian text is a draft | project owner: sign-offs; qa: accessibility runs; product-design and frontend-mobile: store forms and privacy page |
| 13 | Docs | User docs, changelog, support hand-off | R | Partial | docs | README, CONTRIBUTING, SECURITY, LICENSE exist; no user guide, no changelog, no store listing text, no privacy page in `docs/` | product-manager |
| 14 | Dependencies | Licences, failure modes understood | R | Partial | docs | All reviewed packages have permissive licences (`dependency-review.md`); versions pinned except two; no automated vulnerability scan; the SQLite build hook downloads a prebuilt binary at build time and checks its hash | frontend-mobile: scan step in CI |
...
4. **Independent review** of the encryption and export design (ADR-001, ADR-003, RISK-005), even a friend who reads code.
5. **Accessibility runs**: TalkBack through the main flows, 200% text on every screen in both languages, contrast on the built screens (TC-087 to TC-089).
6. **Owner sign-offs**: the legal self-assessment, the privacy statement, the Indonesian text.
7. **Store material**: data-safety answers, privacy statement page, listing text, name check.

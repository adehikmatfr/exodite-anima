...
| 2026-09-23 | Implemented ADR-006 rules 1 and 2 (copy before a migration, restore and refuse to leave a half-migrated file on failure) in `app/lib/data/journal_database.dart`; rule 3 was already built. New test `app/test/migration_safety_test.dart` (5 cases) proves the mechanism against a real migration failure, since the schema is still version 1 and there is no real `onUpgrade` step to test against yet (TC-100 now Partial, from not executed). `flutter analyze`: no issues; full suite (109 tests across all files) passes. FEAT-001 AC-11, ADR-006, `local-data-design.md`, and the FEAT-010 effort read updated; FEAT-010's remaining size driver is the real schema-2 migration step itself, not this generic safety net (now built) |

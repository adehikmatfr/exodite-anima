...
- Two more real-device bugs found 2026-09-23, same session, once the owner actually tried the camera and library (v1.1.0-beta.2 fix above was necessary but not sufficient): (1) on a brand-new, still-blank entry, tapping "Add photo" opened nothing at all - `_ensureEntryId()` runs before the picker and silently returns `null` when there is no saved text yet, so the picker call is never reached. Fixed by showing `S.photoNeedsTextFirst` (reusing `_PhotoAddError`, now parameterised with `title`/`body` instead of hardcoded photo-add-failed text) instead of doing nothing. (2) On an existing entry, the camera or library opened, but the picked photo then vanished and the app landed back on the timeline asking for the passcode: opening either one backgrounds the app, and FEAT-003's shell locks *immediately* on backgrounding by default, closing the database and wiping the key before `image_picker`'s result (delivered only after resuming) could ever reach `EntryRepository.addPhoto`. Fixed with `AutoLockGuard` (`app/lib/lock/auto_lock_guard.dart`): `_addPhoto` holds a counter for its whole picker-and-save operation; `_ShellState._lock()` (`app/lib/main.dart`) defers while any hold is active and applies the deferred lock the instant the last one ends - the passcode is still asked for again afterward, only the photo now lands in the database before that happens. Full mechanism, the residual security trade-off (RISK-014), and why the photo-library needs no extra Android permission while the camera does (`CAMERA` added to `AndroidManifest.xml` and `tool/check_release_manifest.sh`'s allow-list) are recorded in `../adr/ADR-002-local-database.md`'s 2026-09-23 update and `../report/permission-and-privacy-register.md`. Proven in `app/test/auto_lock_guard_test.dart` (the guard in isolation, plus a real `JournalApp` integration case); `flutter analyze` clean, full suite 150 tests. Not yet re-verified on a real device.

...
Given by frontend-mobile (the assistant acting in this role) on 2026-09-20 for `write-feature-spec` step 9. It answers FEAT-001 to FEAT-009, plus FEAT-010 and FEAT-011 added 2026-09-23. Status: relative sizes only, not measured, not a schedule.
...
| FEAT-010 | Tags, mood, and On this day | L (built) | Updated 2026-09-23, several times across the day. All nine acceptance criteria are now built: the schema step (`onUpgrade` 1 -> 2, `mood`/`tags`/`entry_tags`), the repository layer, the editor's mood picker and tag input, the timeline's display of mood/tags and the "On this day" card, and export/import carrying the new fields (ADR-003's `schemaVersion` 2). Checking screen privacy (AC-9) also found and fixed a real, pre-existing FEAT-003 bug (THR-004: the privacy cover did not block the accessibility tree). Proven against a genuine hand-built schema-1 fixture and 122 tests total (unit, widget, and format level). What is left: any widget or emulator test of the real screens (`TimelinePage`, `EditorPage`, the export/import pages), and any run on a device | ADR-002 (update 2026-09-23), ADR-003 (update 2026-09-23), ADR-006 (update 2026-09-23), FEAT-001, FEAT-002, FEAT-006, FEAT-007 | Migration keeps an encrypted copy until the first successful launch on schema 2 and restores it on failure (ADR-006, built); a database on a schema newer than the app knows is refused, never opened for writing (built) |

| FEAT-011 | Photos | L | Given 2026-09-23. Real device integration this project has not needed before: camera and photo-library pickers on both platforms, decoding and compressing arbitrary image files, a new encrypted-file-per-photo store next to the existing database (`media` table plus one AES-256-GCM file per photo, ADR-002 update), a real schema step (`journalSchemaVersion` 2 -> 3, ADR-006 again), and export/import learning to carry binary files, not just JSON (ADR-003 update, the importer's rule-3 file allow-list needs real code). No compression target, no image-picker package, and no camera-permission handling exist anywhere in the app today, unlike FEAT-010's mood/tags UI which only needed widgets. Low confidence until a spike proves the picker and compression path on a real device | ADR-001, ADR-002 (update 2026-09-23), ADR-003 (update 2026-09-23), ADR-006, FEAT-001, FEAT-006, FEAT-007, FEAT-010 | Photos are encrypted the same way media was always meant to be (ADR-002); no double encryption between the device key and an export password; migration follows ADR-006 like every schema step since FEAT-010 |
...
| Whether the copy-before-migrate/restore-on-failure implementation (ADR-006) works cleanly with the encrypted database on both platforms | FEAT-010 | Proven on the host against a real schema-1 fixture, 2026-09-23 (`app/test/migration_safety_test.dart`); not spiked on a physical device or on iOS |
